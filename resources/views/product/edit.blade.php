...
@section('content')
    <main class="col-sm-9 ml-sm-auto col-md-10 pt-3" role="main">
        ...
        @endif
        <form method="post" action="/product/{{ $data['product']['id'] }}">
            {{ csrf_field() }}
            <input type="hidden" name="_method" value="PUT">
            <div class="form-group row">
                <label for="description" class="col-sm-2 col-form-label">产品名称</label>
                <div class="col-sm-10">
                    <input type="text" name="description" value="{{ $data['product']['description'] }}" class="form-control" id="description" placeholder="产品名称">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    ...
                    <img class="displayProduct" src="{{ $data['product']['image'] }}" alt="" id="productImgDisplay">
                    <input type="hidden" name="image" value="{{ $data['product']['image'] }}" class="form-control" id="productImgSubmit">
                </div>
            </div>
            <div class="form-group row">
